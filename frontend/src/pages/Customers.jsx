                            ...
                            <AnimatePresence>
                                {customers.map((c, i) => (
                                    <motion.tr key={c._id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.05 }} className="border-t border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                                        ...
                                        <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs font-bold ${c.customerType === 'B2B' ? 'bg-blue-900 text-blue-200' : 'bg-green-900 text-green-200'}`}>{c.customerType}</span></td>
